...
def iter_all_funding_rates(
    ...
    while t < end_ms:
        batch = fetch_funding_rates(session, symbol, t, end_ms, limit=1000)
        if not batch:
...
def write_csv_gz(path: str, rows: Iterable[FundingRate]) -> int:
    ...
    with gzip.open(path, "wt", newline="") as f:
        w = csv.writer(f)
        w.writerow(["timestamp", "fundingRate", "symbol"])
        for fr in rows:
...
def main() -> int:
    ...
    ap.add_argument("--out", required=True, help="Output path, e.g. data/raw/futures/BTCUSDT_funding.csv.gz")
    ...
    print(f"Downloading funding rates for {args.symbol} from {args.start} to {args.end}")
    rows = iter_all_funding_rates(args.symbol, start_ms, end_ms, throttle_s=args.throttle)
    n = write_csv_gz(args.out, rows)
    print(f"Wrote {n} funding rate records to {args.out}")
    return 0
