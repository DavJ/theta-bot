...
def build_dataset(rows, window, variant, model,
    ...
    for i in range(window, len(rows)-1):
        ...
        if variant == 'raw':
            ...
            f = theta_magphase_features(seg, K=theta_K, tau_re=0.0, ridge=theta_ridge, gram=theta_gs)

        elif variant == 'theta2D':
            # 2D = Gaussovské okno (tau_re>0), bez imag. složky (tau_re=0)
...
def walk_forward_eval(rows, variant, model, window=64, train_frac=0.7, step=256,
    ...
    X, y, meta, Xc_all = build_dataset(
        ...
        theta_phi_scale=theta_phi_scale, 
...
def main():
    ...
    ap.add_argument('--theta-phi-scale', type=float, default=0.002)
    ap.add_argument('--theta-K', type=int, default=48)
    ap.add_argument('--theta-tau', type=float, default=0.12)
            ...
            for up in upper_grid:
                for lo in lower_grid:
                    res = walk_forward_eval(
                        ...
                        theta_phi_scale=args.theta_phi_scale,
