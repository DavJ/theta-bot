...
def fft_features(series, top_n=16, mode='magphase'):
    """FFT -> real-valued features.
    mode:
      - 'abs':      [top magnitudes]
      - 'reim':     [Re_k, Im_k] for top bins (no spectrum rescaling; only time-series z-score)
      - 'magphase': [mag_k, cos(phi_k), sin(phi_k)] for top bins (default; phase-stable)
    """
    x = np.asarray(series, dtype=float)
    ...
    if mode == 'abs':
        ...
        feats = np.concatenate([sel.real, sel.imag])
    else:
        ph = np.angle(sel)
        feats = np.concatenate([mags[idx], np.cos(ph), np.sin(ph)])
...
def wavelet_features(series, levels=3):
    ...
    for _ in range(levels):
        a, d = haar_dwt_level(a)
        feats += [a.mean(), a.std(), d.mean(), d.std(), (d**2).sum()]
...
class ThetaBasis:
    def __init__(self, K=16, tau_im=0.25):
        self.K = K; self.tau_im = tau_im
        self.phases = np.linspace(0, 1, K, endpoint=False)
...
def theta_features(series, K=16, tau_im=0.25, gram=False):
    ...
    T = len(s)
    tb = ThetaBasis(K=K, tau_im=tau_im)
    Phi = tb.design(T)
    if gram:
        Q = np.zeros_like(Phi)
        for i in range(Phi.shape[1]):
            v = Phi[:,i].copy()
            for j in range(i):
                v -= np.dot(Q[:,j], Phi[:,i]) * Q[:,j]
            Q[:,i] = v / (np.linalg.norm(v) + 1e-12)
        Phi = Q
    a, *_ = np.linalg.lstsq(Phi, s, rcond=None)
    recon = Phi @ a
    resid = s - recon
    feats = np.concatenate([a, [resid.mean(), resid.std(), np.sum(resid**2)]])
    feats = np.nan_to_num(feats, copy=False)
    return feats
