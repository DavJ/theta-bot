...
def _read_price_csv(path: str) -> pd.DataFrame:
    ...
    tcol = None
    for c in ["time", "timestamp", "date", "datetime", "Time", "Timestamp", "open_time"]: # Přidán open_time
        if c in df.columns:
            tcol = c
            break
    if tcol is None:
    ...
    for c in ["close", "Close", "price", "Price", "close_price"]:
        if c in df.columns:
            ...
            break
    if ccol is None:
        # Pokud nenajdeme 'close', zkusíme 'open' jako fallback
        for c_open in ["open", "Open", "open_price"]:
    ...
    out.columns = ["time", "close"] # Přejmenujeme vždy na "time" a "close"

    # Robustnější parsování času
    try:
        # Zkusíme nejprve jako string (ISO formát)
        out['time'] = pd.to_datetime(out['time'], utc=True, errors='coerce')
        # Pokud selže a je to číslo, zkusíme jako milisekundy (běžné u crypto dat)
        if out['time'].isnull().any():
             if pd.api.types.is_numeric_dtype(df[tcol]):
                  print(f"Info: Parsing time column in '{path}' as milliseconds since epoch.")
                  # Znovu načteme původní sloupec a parsujeme jako ms
                  out['time'] = pd.to_datetime(df[tcol], unit='ms', utc=True, errors='coerce')
...
def _make_period_grid(minP: int, maxP: int, nP: int) -> np.ndarray:
    ...
    # Použijeme np.unique pro odstranění duplicit po zaokrouhlení
...
def ridge(X, y, lam):
    ...
    if n_features > 0:
        # A.flat[::n_features+1] += lam # Efektivnější, ale méně čitelné
    ...
    try:
        ...
        try:
            # Použijeme A (s regularizací) pro lstsq
            beta = np.linalg.lstsq(A, b, rcond=None)[0]
        except np.linalg.LinAlgError:
            # print("ERROR: lstsq also failed in ridge. Returning zeros.")
            beta = np.zeros(n_features)
        except ValueError as ve: # Zachytí chyby tvaru (např. pokud b je prázdné)
...
def _corr(a: np.ndarray, b: np.ndarray) -> float:
    ...
    # Použijeme robustnější kontrolu (rozptyl) místo allclose
    if np.var(a_clean) < 1e-12 or np.var(b_clean) < 1e-12:
        return 0.0
    try:
        # Výpočet korelace
        corr_matrix = np.corrcoef(a_clean, b_clean)
        # Zkontrolujeme, zda výsledek je platný (někdy může vrátit skalár?)
        if isinstance(corr_matrix, np.ndarray) and corr_matrix.shape == (2, 2):
            return float(corr_matrix[0, 1])
        else:
            # print(f"Warning: Unexpected result from np.corrcoef: {corr_matrix}")
            return float("nan")
    except Exception as e:
...
def _safe_stem_key(path: str) -> str:
    ...
    # Zkrátíme, pokud je příliš dlouhý (pro některé filesystémy)
    max_len = 50
...
def metrics(last_prices, pred_delta, true_delta):
    ...
    # Maska validních hodnot pro predikce a skutečné hodnoty
    valid_mask = ~np.isnan(pred_delta) & ~np.isnan(true_delta)
...
def evaluate_symbol_csv(path: str,
    ...
    df_in = _read_price_csv(path) # Použijeme opravený _read_price_csv
    times = df_in["time"].to_numpy()
    closes = df_in["close"].to_numpy().astype(float)
    n_total = len(closes)

    # 1. Příprava cílové veličiny (y_target)
    y_target = np.full(n_total, np.nan) # Inicializace NaN
    if target_type == 'logprice':
        ...
        if n_total > 1:
            ...
            # diff zachová délku tím, že první prvek je NaN
    ...
    start_idx = window # Potřebujeme historii [t0-window : t0]
    ...
    for t0 in range(start_idx, n_total - horizon):
        ...
        # Zvýšena podmínka na D+1 pro lepší stabilitu regrese
        min_samples_needed = max(8, D + 1)
        if current_window_size < min_samples_needed:
            # print(f"Skipping t0={t0}: Not enough samples ({current_window_size}) < {min_samples_needed}")
            continue
        ...
        if ema_alpha > 0:
            time_weights_raw = np.exp(ema_alpha * (np.arange(n_clean) - (n_clean - 1)))
        ...
        if D > 0:
            ...
            if B_win_fully_weighted.shape[0] >= D:
                 ...
                 continue
        # Pokud D=0, beta zůstane [0] a pred_y bude 0
        ...
        pred_y = 0.0
        if D > 0:
             # Ošetření nekonzistentních tvarů beta (pokud ridge selhal a vrátil špatný tvar)
             if beta_raw_weighted.shape[0] == D:
                 try:
                      # Vážený vstup pro predikci
                      pred_y = float( (b_raw_now * q_weights_sqrt) @ beta_raw_weighted )
                 except ValueError as ve:
                      print(f"Skipping t0={t0}: ValueError during prediction dot product ({ve}). Shapes: b_now*q={ (b_raw_now * q_weights_sqrt).shape}, beta={beta_raw_weighted.shape}")
                      continue
             else:
                 # print(f"Skipping t0={t0}: beta shape mismatch ({beta_raw_weighted.shape[0]} vs {D}).")
                 continue


        # --- Převod predikce y na predikci delty ---
        last_price = closes[t0]
        if np.isnan(last_price) or last_price <= 0: # Přidána kontrola na nekladnou cenu pro log
             # print(f"Skipping t0={t0}: Invalid last_price ({last_price}).")
             continue
        ...
        try:
             if target_type == 'logprice':
                 ...
                 # Kontrola na extrémní hodnoty po exp()
                 if not np.isfinite(pred_future_price): raise OverflowError("exp overflow")
                 pred_delta = pred_future_price - last_price
             elif target_type == 'logret':
                 pred_logret = pred_y
                 # Omezíme exp(pred_logret), aby nedošlo k overflow/underflow
                 exp_pred = np.exp(np.clip(pred_logret, -10, 10)) # Omezení např. na +/- 22000%
        ...
        # Nulu považujeme za nesprávnou predikci směru
        ...
        # Přidáme pouze validní hodnoty do seznamů pro finální metriky
    ...
    if not rows:
        # Zkusíme zjistit, proč nejsou řádky
        ...
        if n_total < window + horizon + 1:
             ...
             print(f"  Reason: Could be due to NaNs in data or parameters causing skips (e.g., small window, large D).")
        # Vrátíme prázdný dataframe a prázdné summary místo chyby
    ...
    # Metriky (použijeme uložené seznamy validních hodnot)
    ...
    summary = {
        ...
        "mae_price": getattr(res, 'mae_price', float('nan')), # MAE delty
        "mae_return": getattr(res, 'mae_return', float('nan')), # MAE návratnosti
    ...
    try:
        class NpEncoder(json.JSONEncoder):
            def default(self, obj):
                ...
                if isinstance(obj, np.floating): return float(obj) if np.isfinite(obj) else None # Nahradí NaN/inf za null
...
def run_batch(symbol_paths: str, **eval_kwargs):
    ...
    for sym in syms:
        ...
        try:
            ...
            # Přidáme jen pokud summary není prázdné (pokud evaluate selhalo tiše)
            if summary:
                rows.append(summary)
        except RuntimeError as re: # Zachytí chyby z nedostatku dat
             print(f"RUNTIME ERROR processing {sym}: {re}")
             rows.append({"symbol": sym, "error": str(re)})
        except ValueError as ve: # Zachytí chyby při načítání/přípravě dat
             print(f"VALUE ERROR processing {sym}: {ve}")
             rows.append({"symbol": sym, "error": str(ve)})
        except Exception as e: # Obecné chyby
            print(f"UNEXPECTED ERROR processing {sym}: {e}")
            rows.append({"symbol": sym, "error": f"Unexpected: {e}"})
...
def parse_args():
    ...
    p.add_argument("--interval", default="1h") # Jen pro info
    p.add_argument("--phase", default="theta_q") # Jen pro info
...
def main():
    ...
    try:
        ...
        # Můžeme zde skončit, nebo pokračovat a doufat, že zápis projde
        # sys.exit(1)
    ...
    if not df_summary.empty:
        # Zkontrolujeme, zda vůbec nějaké řádky nemají chybu
        success_rows = df_summary[~df_summary.apply(lambda row: 'error' in row and pd.notna(row['error']), axis=1)]
        ...
        if not success_rows.empty:
            ...
            if 'lam' in success_rows.columns:
                 df_summary = df_summary.rename(columns={"lam": "lambda"}) # Přejmenujeme v celém df
