...
Optionally orthonormalizes the basis (complex Gram–Schmidt).
...
def gram_schmidt_complex(vectors):
    """Performs complex Gram–Schmidt orthonormalization."""
    ortho = []
    for v in vectors:
        v = v.astype(np.complex128).flatten()
        for u in ortho:
            v = v - np.vdot(u, v) * u  # komplexní vnitřní součin
        norm = np.sqrt(np.vdot(v, v))
        if norm < 1e-12:
            continue
        v = v / norm
        ortho.append(v)
...
def main():
    parser = argparse.ArgumentParser(description="Complex Theta Transform visualizer with orthonormalization")
    ...
    parser.add_argument("--orthonormalize", action="store_true",
                        help="Perform complex Gram–Schmidt orthonormalization over basis")
    ...
    if args.orthonormalize:
        ...
        Theta_vecs = [Theta.flatten()]
        # for now single component; extendable for multiple Θ_i
        ortho_basis = gram_schmidt_complex(Theta_vecs)
        Theta = ortho_basis[0].reshape(Theta.shape)
        print("✅ Basis orthonormalized.")
