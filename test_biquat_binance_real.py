...
def download_binance_data(symbol, interval='1h', limit=2000, output_dir='real_data', use_mock_if_fail=True):
    ...
    Returns path to downloaded CSV file or generates mock data if unavailable.
    """
    ...
    if os.path.exists(output_path):
        try:
            ...
            if len(df) >= limit * 0.9:  # At least 90% of requested data
                print(f"  ✓ Found existing data: {output_path} ({len(df)} samples)")
                return output_path
        except:
            pass
    ...
    try:
        ...
        if result.returncode == 0 and os.path.exists(output_path):
            df = pd.read_csv(output_path)
            print(f"  ✓ Downloaded {len(df)} samples to {output_path}")
            return output_path
        else:
            ...
            if use_mock_if_fail:
                return generate_realistic_market_data(symbol, n_samples=limit, output_dir=output_dir)
            return None
...
"""
    ...
    # Real data results
    real_results = [r for r in all_results if r.get('data_type') == 'real']
    if real_results:
        html += """
        <h2>📈 Real Market Data Results</h2>
        <div class="info-box">
            Real market data from Binance. Performance above 0.5 hit rate indicates 
            genuine predictive power beyond random chance.
        </div>
"""
        
        # Group by pair
        pairs = sorted(set(r['pair'] for r in real_results if 'pair' in r))
        
        for pair in pairs:
            pair_results = [r for r in real_results if r.get('pair') == pair]
            
            html += f"""
        <h3>{pair_results[0].get('pair_label', pair)}</h3>
        <table>
            <tr>
                <th>Horizon</th>
                <th>Hit Rate</th>
...
"""
    ...
    if real_results:
        ...
        md += "Real market data from Binance. Performance above 0.5 hit rate indicates genuine predictive power.\n\n"
        
        pairs = sorted(set(r['pair'] for r in real_results if 'pair' in r))
        
        for pair in pairs:
            pair_results = [r for r in real_results if r.get('pair') == pair]
            md += f"\n### {pair_results[0].get('pair_label', pair)}\n\n"
...
""")
    ...
    if not args.skip_download:
        print("\nDownloading real market data from Binance...")
    
    test_pairs = TRADING_PAIRS[:3] if args.quick else TRADING_PAIRS
    
    for symbol, label in test_pairs:
        if not args.skip_download:
            data_path = download_binance_data(symbol, interval='1h', limit=DATA_LIMIT)
        else:
    ...
    if all_results:
        ...
        real_results = [r for r in all_results if r.get('data_type') == 'real']
        current_pair = None
        for r in sorted(real_results, key=lambda x: (x.get('pair', ''), x.get('horizon', 1))):
            if r.get('pair') != current_pair:
                current_pair = r.get('pair')
                print(f"\n  {r.get('pair_label', current_pair)}:")
            print(f"    h={int(r.get('horizon', 1)):2d}: Hit Rate={r.get('hit_rate', 0):.4f}, "
                  f"Corr={r.get('corr_pred_true', 0):.4f}, N={int(r.get('n_predictions', 0))}")
