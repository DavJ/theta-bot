...
# Jednoduchý wrapper: stáhne ceny (pokud chybí) a spustí trading simulaci.
# Použití: bash scripts/trade_sim.sh BTCUSDT,ETHUSDT 1h
...
python theta_trading_addon/robustness_suite_v4_trade.py \
  --symbols "$SYMS" \
  --interval "$INTERVAL" --window 256 --horizon 4 \
